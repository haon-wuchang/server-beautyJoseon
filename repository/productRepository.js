...
export const getList = async (category_id) => {
    let sql = `
        SELECT pid, pname, category_id, price, discount_rate,
               CONCAT('http://3.36.132.223:9000/', main_image->>'$[0]') AS image,
               main_image, pdate
        FROM product
    `;
...
export const getProduct = async (pid) => {
    ...
    select 
            ...
            p.pdate as pdate,
                concat('http://3.36.132.223:9000/', p.main_image->>'$[0]') as mainImg,
                (
                    select json_arrayagg(concat('http://3.36.132.223:9000/', s.slideCol))
                    from json_table(p.slide_image, '$[*]' columns(slideCol varchar(100) path '$')) as s
                ) as SlideImgList,
                (
                    select json_arrayagg(concat('http://3.36.132.223:9000/', d.descCol))
